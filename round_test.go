...
func TestRound2(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		halfEven, _ := strconv.ParseFloat(tc.HalfEven, 64)
		require.NoError(t, err, "fail to parse "+tc.Input)
		require.Equal(t, up, Round(input, 2, UP), tc.Input)
		require.Equal(t, down, Round(input, 2, DOWN), tc.Input)
		require.Equal(t, ceiling, Round(input, 2, CEILING), tc.Input)
		require.Equal(t, floor, Round(input, 2, FLOOR), tc.Input)
		require.Equal(t, halfUp, Round(input, 2, HALF_UP), tc.Input)
